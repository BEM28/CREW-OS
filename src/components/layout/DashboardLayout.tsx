...
import { 
  ...
  FolderKanban, Users, MessageSquareQuote, FileText, Settings, User, Monitor
} from 'lucide-react';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!user?.uid || !user?.activeCompanyId) return;
    const q = query(
      collection(db, 'users', user.uid, 'companies', user.activeCompanyId, 'projects'),
      ...
      <aside className="w-64 border-r border-border bg-surface flex flex-col">
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3">
          ...
          {recentProjects.length > 0 && (
            <div className="px-3">
              <h3 className="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-3 px-1">Active War Rooms</h3>
                ...
                {recentProjects.map(p => (
                  <Link 
                    ...
                    className={clsx(
                      ...
                      location.pathname.includes(p.id) ? "bg-primary/10 text-primary border-l-2 border-primary ml-1" : "text-text-secondary hover:text-text-primary hover:bg-white/[0.01]"
                    )}
                  >
